...
class ITunesSearchAPI {
  ...
  Future<Map?> lookupByBundleId(final String bundleId,
      {String? country = 'US', final bool useCacheBuster = true}) async {
    ...
    if (debugLogging) {
      debugPrint('upgrader: download: $url');
    }

    try {
      final response = await client!.get(Uri.parse(url));
      if (debugLogging) {
        debugPrint('upgrader: response statusCode: ${response.statusCode}');
      }
      ...
      if (debugLogging) {
        debugPrint('upgrader: lookupByBundleId exception: $e');
      }
      return null;
    }
  }
  ...
  Future<Map?> lookupById(final String id,
      {final String country = 'US', final bool useCacheBuster = true}) async {
    if (id.isEmpty) {
      return null;
    }
  ...
  /// ```lookupURLByBundleId('com.google.Maps');```
  /// ```lookupURLByBundleId('com.google.Maps', country: 'FR');```
  String? lookupURLByBundleId(final String bundleId,
      {final String country = 'US', final bool useCacheBuster = true}) {
    if (bundleId.isEmpty) {
      return null;
    }
  ...
  /// Look up URL by QSP.
  String? lookupURLByQSP(final Map<String, String?> qsp,
    ...
    qsp.forEach((final key, final value) => parameters.add('$key=$value'));
    if (useCacheBuster) {
  ...
  Map? _decodeResults(final String jsonResponse) {
...
extension ITunesResults on ITunesSearchAPI {
  ...
  String? bundleId(final Map response) {
    String? value;
    try {
      value = response['results'][0]['bundleId'];
    } catch (e) {
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.bundleId: $e');
      }
    }
    return value;
  }
  ...
  String? currency(final Map response) {
    String? value;
    try {
      value = response['results'][0]['currency'];
    } catch (e) {
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.currency: $e');
      }
    }
    return value;
  }
  ...
  String? description(final Map response) {
    String? value;
    try {
      value = response['results'][0]['description'];
    } catch (e) {
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.description: $e');
      }
    }
    return value;
  }
  ...
  Version? minAppVersion(final Map response, {final String tagName = 'mav'}) {
    Version? version;
    try {
      ...
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.minAppVersion : $e');
      }
    }
    return version;
  }
  ...
  String? releaseNotes(final Map response) {
    String? value;
    try {
      value = response['results'][0]['releaseNotes'];
    } catch (e) {
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.releaseNotes: $e');
      }
    }
    return value;
  }
  ...
  String? trackViewUrl(final Map response) {
    String? value;
    try {
      value = response['results'][0]['trackViewUrl'];
    } catch (e) {
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.trackViewUrl: $e');
      }
    }
    return value;
  }
  ...
  String? version(final Map response) {
    String? value;
    try {
      value = response['results'][0]['version'];
    } catch (e) {
      if (debugLogging) {
        debugPrint('upgrader.ITunesResults.version: $e');
      }
    }
    return value;
  }
